"""
Reading images, saving images and modifying images
"""
...
def save_tiff(filename, data, compression="zlib"):
    ...
    """
    imwrite(filename, data, metadata={"mode": "composite"},
            imagej=True, compression=compression)


def save_tiff_from_float(filename, data, compression="zlib"):
    """ save_tiff, with additional converion of the image from [0,1] to 8bit.
    """
    imwrite(filename, (data*255).astype("H"), metadata={"mode": "composite"},
            imagej=True, compression=compression)
...
def claher(img, kernel_size=127, clip_limit=0.01, nbins=256):
    ...
    """
    img = exposure.equalize_adapthist(img, kernel_size=kernel_size,
                                      clip_limit=clip_limit, nbins=nbins)
    img = img / img.max()
    return img


def mclaher(img, kernel_size=[10, 128, 128], n_bins=128, clip_limit=0.01,
            adaptive_hist_range=False):
    """
    Runs Contrast Limited Adaptive Histogram Equalization (CLAHE) in 3D
    and normalizes to [0,1].
    """
    img = mc.mclahe(img, kernel_size=kernel_size,
                    n_bins=n_bins, clip_limit=clip_limit,
                    adaptive_hist_range=adaptive_hist_range)
    img = img / img.max()
    return img
...
def resize_shrink(img, factor):
    ...
    """
    return cv2.resize(img, np.asarray(img.shape)[::-1]//factor,
